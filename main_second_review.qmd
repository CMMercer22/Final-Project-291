...
6.  `School Progress Score` : The School Progress score is calculated from two components, academic growth and relative performance. Academic growth is based on STAAR growth outcomes which is initially reported as a raw score, then converted to a scaled score from 0-100 using a table defined in the handbook. The relative performance component is calculated using STAAR performance outcomes which are also converted to a scaled 0-100 score. The higher of the two is selected as the School Progress score.
